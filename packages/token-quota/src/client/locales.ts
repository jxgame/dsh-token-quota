/**
 * Token-quota panel copy (zh + en). The panel is a product surface, so the
 * primary copy is Chinese with an English pair, mirroring the shipped UI.
 *
 * @module @jxgame2020/dsh-token-quota/client/locales
 */

/** Simplified Chinese copy (primary). */
export const zh = {
  title: '每日 Token 限额',
  subtitle: '按模型实时统计今天的 Token 消耗',
  current: '当前',
  unlimited: '不限',
  usedToday: '今日已用',
  limitLabel: '上限',
  limitPlaceholder: '填数字，0=不限',
  save: '保存',
  select: '选择',
  setLimitHint: '设置限额',
  loading: '加载中…',
  waiting: '等待数据…',
  loadError: '加载失败',
  empty: '暂无模型数据',
  collapse: '收起',
  expand: '展开',
  expandHint: '展开每日 Token 限额面板',
  settings: '设置',
  settingsTitle: '设置',
  settingsTabQuota: '限额设置',
  settingsTabMusic: '音乐设置',
  settingsTabOther: '其它',
  reorderHint: '拖动排序',
  noteNewHint: '新建笔记（最多 5 个）',
  noteMaxHint: '最多 5 个笔记',
  noteUntitled: '未命名',
  noteRenameHint: '点击可修改标题',
  noteCollapse: '收起',
  noteExpand: '展开',
  noteClose: '关闭（内容保留）',
  noteCloseHint: '关闭（内容保留，可从首字图标重新打开）',
  noteDelete: '删除笔记',
  noteResizeHint: '拖动改变大小',
  notePlaceholder: '在这里记录…',
  monitorLabel: '监控模型',
  monitorHint: '未勾选的模型不显示、不计入用量、不受限额限制',
  monitorAll: '全部',
  monitorNone: '无',
  noMonitored: '未监控任何模型',
  fullActionLabel: '满额后处理',
  fullStop: '停止请求并提示',
  fullSwitchQuota: '自动切换到其它限额模型',
  fullSwitchAll: '自动切换（优先限额模型，其次非限额）',
  fullNotice: '当前模型今日额度已用尽，请求已停止。请选择其它模型或调整限额。',
  fullSwitchFailed: '没有可切换的模型，请手动选择其它模型或调整限额',
  fullSwitchTo: '已自动切换到 {model}',
  close: '关闭',
  logs: '日志',
  logsTitle: '用量日志',
  logEmpty: '暂无记录',
  logNoMatch: '无匹配记录',
  logSearchPlaceholder: '搜索模型…',
  logClear: '清除',
  logClearTitle: '清除历史日志',
  logClearHint: '将删除所选日期之前的所有记录（不含当日）',
  logClearConfirm: '确认清除',
  logClearCancel: '取消',
  logClearing: '清除中…',
  logPagerInfo: '第 {from}-{to} 条 / 共 {total} 条',
  logPerPage: '条/页',
  logDay: '日期',
  logModel: '模型',
  logUsed: '用量',
  resetLabel: '每日重置时间',
  resetHint: '选择时区与时间，到点自动清零当日计数',
  resetOffset: '时区',
  resetTime: '时间',
  upgradeAvailableHint: '有新版本可用',
  upgradeAvailable: '发现新版本 v{version}，请在终端执行以下命令升级：',
  copy: '复制',
  copied: '已复制 ✓',
  upgradeHint: '升级后请重启 dsh web 并刷新浏览器页面',
  checkUpdatesLabel: '自动检查更新',
  dimWhenIdleLabel: '失焦时窗口透明',
  dimWhenIdleHint: '鼠标移开面板时降低不透明度，输入文字时更透明；鼠标悬停恢复',
  musicLabel: '动作音乐',
  musicHint: '根据 Harness 动作实时作曲：请求开始、工具调用、回合结束等（需点击开关启动音频；检测到 MIDI 输出时同步发送 MIDI）',
  musicOn: '开',
  musicOff: '关',
  musicVolume: '音量',
  musicStyle: '风格',
  musicStyleMajor: '大调',
  musicStyleMinor: '小调',
  musicStylePentatonic: '古风',
  musicOnlyCurrentSessionLabel: '只播当前会话',
  musicOnlyCurrentSessionHint: '开启后仅当前窗口的会话有音乐，后台子任务静音；关闭则所有会话都参与作曲',
  checkUpdatesNow: '立即检查',
  checkingUpdates: '检查中…',
  checkUpToDate: '已是最新',
  checkFailed: '检查失败',
}

/** Dictionary keys owned by the token-quota panel (a string-literal union). */
export type TokenQuotaKey = keyof typeof zh

/** English copy. */
export const en = {
  title: 'Daily Token Quota',
  subtitle: 'Real-time per-model token usage today',
  current: 'current',
  unlimited: 'unlimited',
  usedToday: 'used today',
  limitLabel: 'limit',
  limitPlaceholder: 'number, 0 = unlimited',
  save: 'Save',
  select: 'Select',
  setLimitHint: 'Set limit',
  loading: 'Loading…',
  waiting: 'Waiting…',
  loadError: 'Load failed',
  empty: 'No model data',
  collapse: 'Collapse',
  expand: 'Expand',
  expandHint: 'Expand the daily token quota panel',
  settings: 'Settings',
  settingsTitle: 'Settings',
  settingsTabQuota: 'Quota',
  settingsTabMusic: 'Music',
  settingsTabOther: 'Other',
  reorderHint: 'Drag to reorder',
  noteNewHint: 'New note (max 5)',
  noteMaxHint: 'At most 5 notes',
  noteUntitled: 'Untitled',
  noteRenameHint: 'Click to rename',
  noteCollapse: 'Collapse',
  noteExpand: 'Expand',
  noteClose: 'Close (keep content)',
  noteCloseHint: 'Close but keep the content; reopen from its chip',
  noteDelete: 'Delete note',
  noteResizeHint: 'Drag to resize',
  notePlaceholder: 'Write here…',
  monitorLabel: 'Monitored models',
  monitorHint: 'Unchecked models are hidden, not metered, and never capped',
  monitorAll: 'All',
  monitorNone: 'None',
  noMonitored: 'No model monitored',
  fullActionLabel: 'When a model is full',
  fullStop: 'Stop and prompt',
  fullSwitchQuota: 'Switch to another quota model',
  fullSwitchAll: 'Switch models (quota models first, then uncapped)',
  fullNotice: 'Today\u2019s quota for the current model is exhausted; the request was stopped. Pick another model or raise its limit.',
  fullSwitchFailed: 'No switchable model \u2014 pick one manually or raise a limit',
  fullSwitchTo: 'Auto-switched to {model}',
  close: 'Close',
  logs: 'Logs',
  logsTitle: 'Usage Log',
  logEmpty: 'No records',
  logNoMatch: 'No matching records',
  logSearchPlaceholder: 'Search model…',
  logClear: 'Clear',
  logClearTitle: 'Clear historical log',
  logClearHint: 'This deletes all records before the selected date (the date itself is kept).',
  logClearConfirm: 'Confirm',
  logClearCancel: 'Cancel',
  logClearing: 'Clearing…',
  logPagerInfo: '{from}-{to} of {total}',
  logPerPage: '/page',
  logDay: 'Day',
  logModel: 'Model',
  logUsed: 'Used',
  resetLabel: 'Daily reset',
  resetHint: 'Pick a timezone and time; counters reset there',
  resetOffset: 'Timezone',
  resetTime: 'Time',
  upgradeAvailableHint: 'A new version is available',
  upgradeAvailable: 'A new version v{version} is available. Run one of the following commands in your terminal to upgrade:',
  copy: 'Copy',
  copied: 'Copied ✓',
  upgradeHint: 'After upgrading, restart dsh web and refresh the browser',
  checkUpdatesLabel: 'Check for updates automatically',
  dimWhenIdleLabel: 'Dim panel when unfocused',
  dimWhenIdleHint: 'Fade the panel out when the pointer is away (more while typing); hover to restore',
  musicLabel: 'Action music',
  musicHint: 'Live-composes from Harness actions: request starts, tool calls, turn ends, etc. (click the switch to start audio; a detected MIDI output is driven as well)',
  musicOn: 'On',
  musicOff: 'Off',
  musicVolume: 'Volume',
  musicStyle: 'Style',
  musicStyleMajor: 'Major',
  musicStyleMinor: 'Minor',
  musicStylePentatonic: 'Gufeng',
  musicOnlyCurrentSessionLabel: 'Current session only',
  musicOnlyCurrentSessionHint: 'When on, only the visible session drives the soundtrack (background tasks stay silent); when off, every session contributes.',
  checkUpdatesNow: 'Check now',
  checkingUpdates: 'Checking…',
  checkUpToDate: 'Up to date',
  checkFailed: 'Check failed',
} satisfies Record<TokenQuotaKey, string>
