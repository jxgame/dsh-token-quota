...
/**
 * Maximum number of floating scratchpad notes the panel keeps. The new-note
 ...
 * reopened from their header chip.
 */
export interface TokenQuotaNote {
  /** Stable identity, generated by the client when the note is created. */
  id: string
  /** Editable title; also the source of the header chip's first character. */
  title: string
  /** Textarea content. */
  text: string
  ...
  x: number
  y: number
  /** Window size in CSS pixels (ignored while `collapsed`). */
  width: number
  height: number
  /** Collapsed notes render as a title-bar-only strip. */
  collapsed: boolean
  /** Closed notes keep their content and placement but are not rendered. */
  visible: boolean
}
