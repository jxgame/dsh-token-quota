/**
 * Standalone build config for @jxgame2020/dsh-token-quota: the Host node
 * library plus the browser client bundle (closure-factory artifact for the
 * web module loader). No core-Harness build wiring is involved — the client
 * bundle logic below is the same shape the Harness repo inlines for its UI
...
const ID = '@jxgame2020/dsh-token-quota'
...
/** Host node library: emits lib/index.js + lib/invariant.js. */
...
const clientConfig: UserConfig = {
  ...
  plugins: [{
    ...
    resolveId(source: string) {
      ...
      if (GENERATED_REMOTE.test(source)) return null
      throw new Error(
        `client bundle purity: "${source}" is not a platform module (CLIENT_EXTERNALS) or a generated /remote contribution — `
